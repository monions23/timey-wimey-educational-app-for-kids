...
export default function Planet() {
  ...
  if (!planet) {
    return <div className="min-h-screen bg-black text-cream flex items-center justify-center">Planet not found</div>;
  }

  return (
    <div className="min-h-screen bg-black text-cream relative overflow-hidden transition-opacity duration-500" style={{ opacity: animationStage >= 1 ? 1 : 0 }}>
      {/* Back button */}
      ...
      <div className="flex items-center justify-between min-h-screen p-8 gap-12">
        ...
        <div className="w-[550px] bg-navy-blue border-8 border-cream rounded-2xl p-8 relative max-h-[90vh] overflow-y-auto">
          <h1 className="text-center text-5xl text-orange mb-8 uppercase tracking-wider" style={{ fontFamily: 'serif' }}>
            {planet.name}
          </h1>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.size}</div>
            </div>
            <div>
              <div className="text-2xl text-cream mb-2">Distance:</div>
              <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.distanceFromSun}</div>
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.lengthOfDay}</div>
          </div>

          {/* Length of Year */}
          <div className="mb-4">
            <div className="text-2xl text-cream mb-2">Length of Year:</div>
            <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.lengthOfYear}</div>
          </div>

          {/* Tilt */}
          <div className="mb-6">
            <div className="text-2xl text-cream mb-2">Tilt:</div>
            <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.tilt}</div>
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {planet.atmosphere.split(',').map((element, i) => (
                <div key={i} className="bg-cream text-black px-4 py-2 text-sm border-4 border-orange uppercase tracking-wider">
                  {element.trim()}
                </div>
              ))}
            </div>
          ...
          <div className="mb-4">
            ...
            <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.temperature}</div>
          </div>

          {/* Potential for Life */}
          <div className="mb-6">
            <div className="text-2xl text-cream mb-2">Potential for Life:</div>
            <div className="bg-orange text-black px-3 py-3 text-lg border-4 border-cream">{planet.potentialForLife}</div>
          </div>

          {/* Surface */}
          <div className="mb-6">
            <div className="text-2xl text-cream mb-2">Surface:</div>
            <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange">{planet.surface}</div>
          </div>
            ...
            >
              {showLayers && (
                <svg width="30" height="30" viewBox="0 0 30 30">
                  <path d="M 5 15 L 12 22 L 25 9" stroke="var(--color-cream)" strokeWidth="3" fill="none" />
                </svg>
              )}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange text-center">{planet.moons}</div>
            </div>
            <div>
              <div className="text-2xl text-cream mb-2">Rings:</div>
              <div className="bg-cream text-black px-3 py-2 text-lg border-4 border-orange text-center">{planet.rings}</div>
            </div>
          </div>
        </div>
      </div>
